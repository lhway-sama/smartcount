...
const TVA: React.FC = () => {
  ...
  const handleGenerate = async () => {
    setIsLoading(true);
    try {
      const response = await apiClient.get(`/api/rapports/tva/?periode=${periode}`, {
        responseType: 'blob',
      });
      ...
      addNotification({
        type: 'error',
        message: 'Impossible de générer la déclaration TVA.',
      });
    } finally {
      setIsLoading(false);
    }
  ...
  const handleDownload = () => {
    if (pdfUrl) {
      ...
      link.href = pdfUrl;
      link.setAttribute('download', `declaration-tva-${periode}.pdf`);
