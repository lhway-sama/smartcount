...
const Bilan: React.FC = () => {
  ...
  const handleGenerate = async () => {
    setIsLoading(true);
    try {
      const response = await apiClient.get(`/api/rapports/bilan/?periode=${periode}`, {
        responseType: 'blob',
      });
      ...
      addNotification({
        type: 'error',
        message: 'Impossible de générer le bilan.',
      });
    } finally {
      setIsLoading(false);
    }
  ...
  const handleDownload = () => {
    if (pdfUrl) {
      ...
      link.href = pdfUrl;
      link.setAttribute('download', `bilan-${periode}.pdf`);
