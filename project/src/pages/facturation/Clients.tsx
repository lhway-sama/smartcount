...
const Clients: React.FC = () => {
  ...
  const fetchFactures = async () => {
    try {
      const response = await apiClient.get('/api/factures/clients/');
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      if (formData.id) {
        await apiClient.put(`/api/factures/clients/${formData.id}/`, formData);
        addNotification({
          type: 'success',
          message: 'Facture mise à jour avec succès.',
        });
      } else {
        await apiClient.post('/api/factures/clients/', formData);
        addNotification({
          type: 'success',
  ...
  const handleArchive = async (id: number) => {
    ...
    try {
      await apiClient.post(`/api/factures/clients/${id}/archive/`);
      addNotification({
        type: 'success',
